...
- **Sauvegardes chiffrées + hors-site** : `security/backup-postgres.sh` (dump vérifié avant publication, rétention, copie rclone optionnelle vers un stockage objet) et `security/restore-drill.sh` (exercice de restauration : restaure réellement la dernière sauvegarde dans un conteneur jetable et vérifie la base obtenue).
