...
- **Healthcheck du frontend** : le conteneur Next n'en avait aucun. `docker compose ps` et `deploy.sh` ne pouvaient dire que « running » — c'est-à-dire « le processus ne s'est pas arrêté », une information pauvre : un serveur qui démarre puis échoue à servir resterait « running » indéfiniment, et `autoheal` n'aurait rien à redémarrer. La sonde demande maintenant au serveur de **répondre** (`GET /`, page prérendue, sans appel au backend). Le déploiement du 30/08/2026, qui remplaçait toute la chaîne de build — Tailwind 4, Next 16, Turbopack, Node 22 — s'était validé sur ce seul mot.
...
- **Mode hors ligne** : service worker à priorité réseau (`frontend/public/sw.js`) — cache uniquement en repli, version épinglée et purgée à l'activation, HTML jamais servi depuis le cache tant que le réseau répond. Bandeau « Hors ligne » dans l'interface. Les contours départementaux en font désormais partie : tirés d'un dépôt tiers, ils étaient cross-origin, donc ignorés par le service worker — la carte perdait ses départements hors ligne, sans que rien ne le signale.
- **Sauvegardes chiffrées, vérifiées et restaurées** : trois scripts, à planifier ensemble (voir `security/README.md`) — `backup-postgres.sh` (dump vérifié *avant* publication, rétention, copie rclone optionnelle hors-site), `backup-verify.sh` (contrôle quotidien : fraîcheur, déchiffrement, contenu), `restore-drill.sh` (exercice trimestriel : restaure réellement la dernière sauvegarde dans un conteneur vierge et interroge la base obtenue). **Les trois n'alertent que si `WEBHOOK_URL` figure sur leur ligne de cron**, et leur `notify()` avale les erreurs de `curl` — un webhook injoignable ne doit pas faire échouer une sauvegarde réussie, mais cela rend un webhook cassé silencieux. Configuré n'est pas fonctionnel : la marche à suivre pour le vérifier est dans `security/README.md`.
